#!/usr/bin/env python3
"""
decision_mood_mission_rag_v6.py
기분전환 미션 AI + RAG + 감정분류 + 돌발미션 + 카테고리 열매 + 콤보 시스템
v6 변경사항:
  1. 돌발미션 콤보 발동 버그 수정 (check_combo → 0 반환)
  2. 미션 성공 후 열매 쪼개기 질문 제거
  3. 내가 걸어온 길: GPT 인사이트 제거, 카테고리·기분·미션요약·효과 목록 출력
  4. 미션 리스트: 미션요약 + 카테고리 + 메모 추가
"""
import os
import sys
import json
...
CACHE_FILE = Path(__file__).parent / ".index_cache" / "mission_rag_v6_index.json"
...
def show_history(data: dict):
    ...
    for i, f in enumerate(reversed(history), 1):
        ...
        print(f"\n  [{i}] {f.get('timestamp', '')}")
        print(f"      {cat_info['col']}[{cat_info['label']}]{RST}  {short}")
...
def show_journey(data: dict):
    successes = [f for f in data.get("fruits", []) if f.get("success")]
    ...
    for i, f in enumerate(successes, 1):
        ...
        print(f"  [{i}] 카테고리: {cat_info['col']}{cat_info['label']}{RST}   기분: {mood}")
        print(f"      미션: {short}")
        if effect:
            print(f"      효과: {effect}")
        print()
