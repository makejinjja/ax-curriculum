#!/usr/bin/env python3
"""
decision_mood_mission_rag_v3.py
기분전환 미션 AI + RAG + 감정분류 + 돌발미션 + 카테고리 열매 + 콤보 시스템
개선: 감정별 논문 가중치 / 카테고리별 쿼리 특화 / 근거 출처 표시 / 미션 히스토리 중복 방지
"""
import os
import sys
import json
...
CACHE_FILE = Path(__file__).parent / ".index_cache" / "mission_rag_v3_index.json"
...
# ── [개선1] 감정 → 논문 소스 가중치 ─────────────────────────
# 소스명에 키워드가 포함되면 cosine score에 해당 배수를 곱함
...
def load_data() -> dict:
    if DATA_FILE.exists():
        ...
        d.setdefault("mission_history", [])   # [개선4] 히스토리 필드
        return d
...
def _expand_query(mood: str, time_str: str,
    ...
    # [개선2] 감정+카테고리 조합 키가 있으면 특화 쿼리 사용
