...
CAT = {
    "건강":   {"label": "건강",   "color": "#FF6B6B", "emoji": "❤️"},
    "생산성": {"label": "생산성", "color": "#4DABF7", "emoji": "💙"},
    "재미":   {"label": "재미",   "color": "#CC5DE8", "emoji": "💜"},
    "성장":   {"label": "성장",   "color": "#51CF66", "emoji": "💚"},
    "돌발":   {"label": "돌발",   "color": "#FFD43B", "emoji": "⚡"},
}
...
def render_tree_chart(fruits: list) -> go.Figure:
    ...
    for cat, g in groups.items():
        if not g["x"]:
            continue
        info = CAT.get(cat, {"color": "#999", "emoji": "🌱", "label": cat})
...
def category_bar(fruits: list) -> go.Figure | None:
    ...
    fig = go.Figure(go.Bar(
        x=[f"{CAT.get(c,{}).get('emoji','')} {c}" for c in cats],
...
def render_home():
    ...
    c1.metric("🍎 열매", f"{total} / {MAX_FRUITS}")
    c2.metric("✅ 성공", f"{success}회")
    c3.metric("📊 성공률", f"{rate}%")
    combo = data.get("combo_count", 0)
    last  = data.get("last_category")
    c4.metric("🔥 콤보", f"{last} {combo}연속" if combo >= 2 and last else "-")
    ...
    if history:
        ...
        for f in reversed(history[-5:]):
            ...
            st.markdown(
                f"{ok} {info['emoji']} "
                f"<span style='background:{info['color']}33; color:{info['color']}; "
                f"padding:1px 7px; border-radius:4px; font-size:12px; font-weight:600;'>"
                f"{info['label']}</span> &nbsp;"
                f"**{f.get('mission','')}** "
                f"<small style='color:#666;'>· {f.get('timestamp','')}</small>",
...
def _mission_show():
    ...
    cat_info = CAT.get(cat,  {"color": "#999", "emoji": "", "label": cat})
    diff_info= DIFF.get(diff, {"color": "#999", "emoji": "", "card": ""})
    ...
    if is_wc:
        ...
        st.info(f"{emoji} **{emotion_type}** → 목적: **{purpose}**")

    c  = cat_info["color"]
    st.markdown(f"""
    <div class="mission-card" style="background:{c}18; border-left:4px solid {c};">
        <div style="font-size:13px; color:{c}; font-weight:600; margin-bottom:8px;">
            {cat_info['emoji']} {cat_info['label']} &nbsp;·&nbsp; {diff_info['emoji']} {diff} &nbsp;·&nbsp; ⏱ {minutes}분
        </div>
        <div style="font-size:21px; font-weight:700; line-height:1.5;">
            {m.get('mission','')}
        </div>
    </div>
    """, unsafe_allow_html=True)
...
def _mission_result():
    m    = st.session_state.get("current_mission", {})
    cat  = m.get("category", "건강")
    info = CAT.get(cat, {"emoji": "", "label": cat, "color": "#999"})

    st.markdown("## 🏁 미션 결과")
    st.markdown(
        f"<div style='background:{info['color']}18; border-radius:8px; "
        f"padding:14px 18px; font-size:16px; font-weight:600;'>"
        f"{info['emoji']} [{info['label']}] {m.get('mission','')}</div>",
        unsafe_allow_html=True,
    )
    st.divider()
...
def _mission_complete():
    ...
    if success:
        ...
        cat      = m.get("category", "건강")
        diff     = m.get("difficulty", "하")
        cat_info = CAT.get(cat,  {"emoji": "", "label": cat, "color": "#999"})
        diff_info= DIFF.get(diff, {"emoji": "", "card": ""})
        c = cat_info["color"]

        st.markdown(f"""
        <div style="background:{c}22; border-radius:14px; padding:28px;
                    text-align:center; margin:16px 0;">
            <div style="font-size:48px; margin-bottom:10px;">🍎</div>
            <div style="font-size:17px; font-weight:700; margin-bottom:6px;">열매 획득!</div>
            <div style="font-size:14px; color:{c}; margin-bottom:6px;">
                {cat_info['emoji']} {cat_info['label']} &nbsp;·&nbsp; {diff_info['emoji']} {diff}
            </div>
            <div style="font-size:13px; color:#888;">{short}</div>
        </div>
        """, unsafe_allow_html=True)

        if combo_card:
            st.markdown(f"""
            <div style="background:rgba(255,215,0,0.15); border:2px solid #FFD700;
                        border-radius:12px; padding:18px; text-align:center; margin:10px 0;">
                <div style="font-size:22px;">🔥</div>
                <div style="font-weight:700; color:#FFD700; font-size:15px;">
                    {combo}연속 콤보! {combo_card} 획득!
                </div>
            </div>
            """, unsafe_allow_html=True)

        st.info("✂️ 나무를 키우거나 **열매 쪼개기**에서 카드로 변환할 수 있어요!")
    else:
        st.markdown("## 🌱 괜찮아요!")
        st.info("시도했다는 것만으로도 충분합니다. 💪 도전 카드를 드립니다.")
        st.markdown("""
        <div style="border:2px solid #4DABF7; border-radius:12px; padding:22px;
                    text-align:center; margin:10px 0;">
            <div style="font-size:32px;">💪</div>
            <div style="font-weight:700; color:#4DABF7; font-size:16px; margin-top:8px;">
                도전 카드
            </div>
        </div>
        """, unsafe_allow_html=True)
...
def render_history():
    ...
    if not history:
        st.info("아직 기록이 없습니다. 첫 미션을 시작해보세요!")
        return
    ...
    for f in filtered:
        ...
        st.markdown(
            f"{ok} &nbsp;"
            f"<span style='background:{c}33; color:{c}; padding:2px 8px; "
...
def render_journey():
    ...
    c1.metric("전체 시도", f"{total}회")
    c2.metric("성공", f"{scount}회")
    c3.metric("성공률", f"{rate}%")
    ...
    for f in reversed(successes):
        ...
        st.markdown(f"""
        <div class="journey-item" style="background:{c}12; border-left:3px solid {c};">
            <div style="font-size:12px; color:#666; margin-bottom:5px;">
                🕐 {f.get('timestamp','')}
            </div>
            <div style="margin-bottom:4px;">
                <span style="background:{c}33; color:{c}; padding:1px 7px;
                             border-radius:4px; font-size:12px; font-weight:600;">
                    {info['emoji']} {info['label']}
                </span>
                <span style="font-weight:600; font-size:15px; margin-left:8px;">
                    {f.get('mission','')}
                </span>
            </div>
            <div style="font-size:13px; color:#888;">기분: {f.get('mood','')}</div>
            {'<div style="font-size:13px; color:#aaa; margin-top:3px;">💡 ' + effect + '</div>' if effect else ''}
            {'<div style="font-size:13px; color:#aaa; margin-top:3px;">📝 ' + memo + '</div>' if memo else ''}
        </div>
        """, unsafe_allow_html=True)
...
def render_cards():
    ...
    if not cards:
        st.info("보유한 카드가 없습니다. 미션을 완료하면 카드를 획득할 수 있어요!")
        return

    st.caption(f"총 {len(cards)}장")
    st.divider()
    ...
    for idx, (name, meta) in enumerate(card_meta.items()):
        ...
        with cols[idx % 3]:
            st.markdown(f"""
            <div class="card-tile" style="border:2px solid {color}; background:{color}15;">
                <div style="font-size:36px;">{emoji}</div>
                <div style="font-size:14px; font-weight:700; color:{color}; margin:8px 0;">
                    {name}
                </div>
                <div style="font-size:22px; font-weight:800;">× {meta['count']}</div>
            </div>
            """, unsafe_allow_html=True)
...
def render_split():
    ...
    for i, f in enumerate(fruits):
        ...
        cat_info = CAT.get(cat,  {"emoji": "", "label": cat, "color": "#999"})
        diff_info= DIFF.get(diff, {"emoji": "", "card": "", "color": "#999"})
        ...
        with col1:
            st.markdown(
                f"{cat_info['emoji']} "
                f"<span style='background:{c}33; color:{c}; padding:2px 7px; "
                f"border-radius:4px; font-size:12px; font-weight:600;'>{cat_info['label']}</span>"
                f" **{mission}** "
                f"<small style='color:#666;'>· {diff_info['emoji']} {diff} · {ts}</small>",
                unsafe_allow_html=True,
            )
        with col2:
...
def render_sidebar():
    ...
    st.divider()
    data    = get_data()
    total   = len(data["fruits"])
    success = len([f for f in data["fruits"] if f.get("success")])
    rate    = int(success / total * 100) if total > 0 else 0
    st.caption(f"🍎 {total}/{MAX_FRUITS}  ·  ✅ {success}회  ·  📊 {rate}%")

    combo = data.get("combo_count", 0)
    last  = data.get("last_category")
    if combo >= 2 and last:
        info = CAT.get(last, {"color": "#FFD700", "emoji": ""})
        st.markdown(
            f"<div style='text-align:center; color:{info['color']}; "
            f"font-weight:700; margin-top:4px;'>🔥 {last} {combo}연속!</div>",
            unsafe_allow_html=True,
        )
