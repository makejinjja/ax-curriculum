...
def get_mission(
    ...
    if fruits:
        ...
        cat_counts = Counter(f.get("category", "") for f in fruits if f.get("category"))
        total = sum(cat_counts.values())
        if total >= 3:
            overused = [c for c, n in cat_counts.items() if n / total > 0.5]
            if overused:
                overused_hint = f"\n- 현재 '{', '.join(overused)}' 카테고리 비율이 높으니 다른 카테고리를 선택해라"
