...
def render_tree_chart(fruits: list):
    ...
    _tree_img = Path(__file__).parent.parent / "img" / "tree.jpg"

    if n == 0:
        if _tree_img.exists():
            st.image(str(_tree_img), width=220)
    ...
    with left_col:
        if _tree_img.exists():
            st.image(str(_tree_img), caption=f"{label}  {n}/{cap}", use_column_width=True)
        else:
    ...
    with right_col:
        ...
        st.markdown("""
        <div style="text-align:left;margin-top:4px;font-size:12px;color:#888;">
